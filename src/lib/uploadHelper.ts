...
export async function uploadProcessedImage(
  ...
  console.log(`[AdminUpload] Archiving ${baseId} to ${folderPath}`);

  // Upload the original file directly as a raw image to avoid transformations.
  // This prevents browser memory issues during bulk uploads and supports all native formats.
...
export async function uploadBatch(
  ...
  for (let i = 0; i < filesArray.length; i++) {
    ...
    try {
      ...
      // Continue with others or throw? For now, we continue.
    }
  }
